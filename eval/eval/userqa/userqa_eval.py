...
def process(line, args, tokenizer, image_processor, model_config):
    ...
    if line["image"] is None:
        ...
        image = Image.open(line["image"]).convert('RGB')
...
def eval_model(args):
    ...
    userqa_dir = os.environ.get('USERQA_DIR', args.input_dir)
    data = pd.read_parquet(os.path.join(userqa_dir, 'user-questions.parquet'))
    img_root = os.path.join(userqa_dir, 'images_v2')
    data['index'] = data.index
    data['image'] = data['index'].apply(lambda x: f"{img_root}/{x}.jpg")
    data = data[data['image'].apply(lambda x: os.path.isfile(x))]
